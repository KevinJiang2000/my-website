...
If I told you that just one second of EEG data can contain thousands of data points, you would probably agree that this is no trivial task. It is certainly beyond what the human eye can discern. Fortunately, with the help of machine learning algorithms, we can allow machines to extract features from the data and compute something akin to a “distance” metric to quantify the similarity between neural responses under different conditions. If two stimulus conditions produce similar neural responses, it is highly likely that they share underlying visual mechanisms.
